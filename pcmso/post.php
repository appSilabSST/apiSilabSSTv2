...
if ($authorization) {
    try {
        if (
            ...
            $sql = "
            INSERT INTO pcmso (nr_pcmso, id_empresa, grau_risco_empresa, id_status_documento, id_local_atividade, grau_risco_local_atividade, id_profissional, data_inicio, data_fim, responsavel, responsavel_cpf, responsavel_email) 
            SELECT 
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $result = array(
                    'status' => 'success',
                    'result' => 'PCMSO cadastrado com sucesso!'
                );
            } else {
                http_response_code(500);
                $result = array(
                    'status' => 'fail',
                    'result' => 'Falha ao cadastrar PCMSO!'
                );
            }
        } else {
